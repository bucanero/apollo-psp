...
static void _draw_OptionsMenu(u8 alpha)
{
    ...
    for (int ind = 0, y_off = 48; menu_options[ind].name; ind++, y_off += APP_LINE_OFFSET)
    {
        SetFontColor(APP_FONT_COLOR | alpha, 0);
        DrawString(MENU_ICON_OFF + MENU_TITLE_OFF + 10, y_off, menu_options[ind].name);
		...
		switch (menu_options[ind].type)
		{
			...
			case APP_OPTION_CALL:
				DrawTextureCenteredX(&menu_textures[pspPadGetConf()->crossButtonOK ? footer_ico_cross_png_index : footer_ico_circle_png_index], OPTION_ITEM_OFF, y_off+4, 0, menu_textures[footer_ico_cross_png_index].width /2, menu_textures[footer_ico_cross_png_index].height /2, 0xFFFFFF00 | alpha);
				break;
