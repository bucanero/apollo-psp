...
void Draw_CheatsMenu_Options_Ani(void)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawScrollBar(menu_old_sel[MENU_CODE_OPTIONS], list_count(selected_centry->options[option_index].opts), APP_LINE_OFFSET, SCREEN_WIDTH - 30, game_a);
...
int DrawCodes(code_entry_t* code, u8 alpha, int y_inc, int xOff, int selIndex)
{
    ...
    if (code->file && (code->type == PATCH_BSD || code->type == PATCH_GAMEGENIE))
        DrawFormatString(xOff + MENU_ICON_OFF + 20, SCREEN_HEIGHT - 50, "%s %s", _("Target File:"), code->file);
...
void Draw_CheatsMenu_View_Ani(const char* title)
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
		...
		DrawScrollBar(menu_old_sel[MENU_PATCH_VIEW], nlines, APP_LINE_OFFSET, SCREEN_WIDTH - 30, game_a);
...
//			if (item->flags & SAVE_FLAG_PSV) tmp[0] = CHAR_TAG_PSV;
			tmp[1] = (item->type == FILE_TYPE_VMC) ? CHAR_TAG_VMC : ' ';
