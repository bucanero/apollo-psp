	...
	switch (id) //going to menu
	{
		...
		case MENU_PATCHES: //Cheat Selection Menu
			...
			if (selected_entry->flags & SAVE_FLAG_ONLINE)
			{
				...
				if (selected_entry->flags & SAVE_FLAG_PS1 && file_exists(iconfile) != SUCCESS)
					http_download(selected_entry->path, "icon0.png", iconfile, 1);
			}
...
static int updatePadSelection(int total)
{
	...
	else if (pspPadGetButtonHold(PSP_CTRL_LEFT))
		move_selection_back(total, 5);

	else if (pspPadGetButtonHold(PSP_CTRL_LTRIGGER))
		move_selection_back(total, 25);

//	else if (pspPadGetButtonHold(PSP_CTRL_L2))
//		menu_sel = 0;

	else if (pspPadGetButtonHold(PSP_CTRL_RIGHT))
		move_selection_fwd(total, 5);

	else if (pspPadGetButtonHold(PSP_CTRL_RTRIGGER))
		move_selection_fwd(total, 25);

//	else if (pspPadGetButtonHold(PSP_CTRL_R2))
//		menu_sel = total - 1;
...
static void doSaveMenu(save_list_t * save_list)
{
	...
	else if (pspPadGetButtonPressed(PSP_CTRL_TRIANGLE) && save_list->UpdatePath)
	{
		selected_entry = list_get_item(save_list->list, menu_sel);
	...
	else if (pspPadGetButtonPressed(PSP_CTRL_SQUARE))
	{
...
static void doPatchMenu(void)
{
	...
	else if (pspPadGetButtonPressed(PSP_CTRL_TRIANGLE))
	{
		selected_centry = list_get_item(selected_entry->codes, menu_sel);
